...
public class MinePersonInfoMobileActivity extends Activity implements
		IContentReportor {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.tenement_main_mine_person_mobile);
	...
	private void initView() {
		...
		saveTv.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				mobile = nickEt.getText().toString().trim();
				if (mobile != null && !"".equals(mobile)) {

					if (isMobileNO(mobile)) {

						makeUpdateNickRequest(mobile);
					} else {
						Toast.makeText(MinePersonInfoMobileActivity.this,
								"请填写正确的手机号码", Toast.LENGTH_LONG).show();
					}
				} else {
					Toast.makeText(MinePersonInfoMobileActivity.this,
							"请填写你的手机号码", Toast.LENGTH_LONG).show();
				}
	...
	private void makeUpdateNickRequest(String nick) {
		...
		reqContent
				.appendData(Contants.PROTOCOL_REQ_BODY_DATA.mobilePhone.name(), nick);
		...
		if ("".equals(rtnCode) || rtnCode == null) {
			...
			if (data != null && data instanceof Boolean) {
				...
				if (b.booleanValue()) {
					...
					Toast.makeText(this, "设置失败，请稍后尝试。", Toast.LENGTH_SHORT)
							.show();
				}
			} else {
