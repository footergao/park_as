...
public class CommunitySuggestDtlActivity extends Activity implements
		IContentReportor {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		String  adviceIdStr = getIntent().getStringExtra("adviceId");
		double  advice = Double.parseDouble(adviceIdStr);
		adviceId = (int)  advice+"";

		TextView titleTv = (TextView) this.findViewById(R.id.titile_tv);
		titleTv.setText("更多详情");
	...
	private void getData(String adviceId) {
		...
		reqContent.appendData(Contants.PROTOCOL_REQ_BODY_DATA.operType.name(),
				"000");
		reqContent.appendData(Contants.PROTOCOL_RESP_BODY.adviceId.name(),
				adviceId);

		// SEND
		HttpRequestEntity httpRequestEntity = new HttpRequestEntity(reqContent,
				Contants.SERVER_HOST,
				Contants.PROTOCOL_COMMAND.GET_SUGGEST_DTL.getValue());
	 ...
	 * 初始化数据
	 */
	private void updatedSure(String adviceId) {
		// HEARD
		RequestContentTemplate reqContent = new RequestContentTemplate();
		reqContent.setEncryptoType(Contants.CryptoTyepEnum.aes);// 请求使用AES加密
		...
		HttpRequestEntity httpRequestEntity = new HttpRequestEntity(reqContent,
				Contants.SERVER_HOST,
				Contants.PROTOCOL_COMMAND.UPDATE_SUGGEST_SURE.getValue());
	...
	public void onClick(View view) {
		...
		case R.id.lay_messagecircle_back:
			...
			break;
		case R.id.btn_ok:
			updatedSure(adviceId);
			
			break;
		case R.id.btn_reply:
			Intent intent = new Intent(this,
					CommunitySuggestReplyActivity.class);
			intent.putExtra("replayType", "0");// 消息类型 0:投诉建议，1:设备报修
			intent.putExtra("relationId", adviceId);
			...
			if (RQ_MY_POSTS.equals(responseContent.getResponseCode())) {
				...
				if (jsonStr != null && !"".equals(jsonStr)
					...
					try {
						...
						String propertyAdviceEx = json.get("propertyAdviceEx")
								.toString();

						JSONObject property = new JSONObject(propertyAdviceEx);
						String adviceContent = property.get("adviceContent")
								.toString();
						String files = property.get("files").toString();
						String adviceTime = property.get("adviceTime")
								.toString();
