...
//	        SDCARDPATH = Environment.getExternalStorageDirectory() + "/";  
	    	if (checkSaveLocationExists()) {
				...
				SDCARDPATH=context.getFilesDir()+ "cache/";
			}
	    }  
	...
	public  File createFile(String folderPath, String fileName) {
		File destDir = new File(SDCARDPATH+folderPath);
		if (!destDir.exists()) {
			destDir.mkdirs();
		}
		filePath =SDCARDPATH+folderPath+"/"+fileName;
		Log.v("createFile", "file path--->"+filePath);
		return new File(SDCARDPATH+folderPath, fileName);
	}
	...
	public boolean isFileExist(String fileName) {
		...
		Log.v("isFileExist", "file path--->"+filePath);
		return file.exists();
	}
	/**
	 * 判断SD卡上文件是否存在
