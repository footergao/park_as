...
import com.hdzx.tenement.utils.ResUtil;
import com.hdzx.tenement.utils.UIUtil;
 ...
 */
@SuppressLint("NewApi")
public class CordovaActivity extends Activity implements CordovaInterface, OnClickListener {
    ...
    public final static int UPDATE_PAGE_TITLE = 1007;
    ...
    private void refreshPageTitle() {
        if (null != this.appView) {
            String appviewtitle = this.appView.getTitle();
            if (isDisplayTopBar) {
                appviewtitle = activityTitle;
            }
            if (appviewtitle != null && appviewtitle.length() > 6) {
                String title = appviewtitle.substring(0, 6) + "...";
                tvTitle.setText(title);
            } else
                tvTitle.setText(appviewtitle);
        }
    }
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(ResUtil.getResofR(CordovaActivity.this).getLayout("cordova_main_activity"));
        localinflater = (LayoutInflater) this.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
        llMainContainer = (LinearLayout) findViewById(ResUtil.getResofR(CordovaActivity.this).getId("main_container"));
        llTitlebar = (LinearLayout) findViewById(ResUtil.getResofR(CordovaActivity.this).getId("top_title"));
        tvTitle = (TextView) findViewById(ResUtil.getResofR(CordovaActivity.this).getId("txt_nav_title"));
        LinearLayout btn_back = (LinearLayout) findViewById(ResUtil.getResofR(CordovaActivity.this).getId("lay_nav_back"));
        btn_back.setOnClickListener(this);
        progressbar = (ProgressBar) findViewById(ResUtil.getResofR(CordovaActivity.this).getId("top_progress_bar"));
    }
...
/*        if(activityTitle != null && activityTitle.length() > 6){
        	String title = activityTitle.substring(0, 6) + "...";
        	tvTitle.setText(title);
        }else
...
*/
        ...
        root = new LinearLayoutSoftKeyboardDetect(this, width, height - UIUtil.dip2px(getActivity(), 100));
        ...
        llMainContainer.addView(root);
        appView.setProgressBar(progressbar);
        appView.displayTopBar(isDisplayTopBar);
    ...
    public boolean backHistory() {
        return this.appView != null && appView.backHistory();
    }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent intent) {
        ...
        if (requestCode == REUSLT_FROM_LOGIN) {
    ...
    public void onClick(View v) {
        if (v.getId() == ResUtil.getResofR(CordovaActivity.this).getId("lay_nav_back")) {
            if (null != this.appView)
