...
//    	Log.i("IceCream", "url = " + url);
    	...
    	try {
        	...
        	for(String token : CACHED_URLS){
        		int index = url.indexOf(token);
            	if(index >= 0){
            		newURL = "file:///android_asset/"  +url.substring(index);
            		break;
            	}
        	}

            CordovaResourceApi resourceApi = appView.getResourceApi();
            Uri origUri = Uri.parse(newURL != null ? newURL : url);
            
            // Allow plugins to intercept WebView requests.
            Uri remappedUri = resourceApi.remapUri(origUri);
            
            if (newURL != null || !origUri.equals(remappedUri) || needsSpecialsInAssetUrlFix(origUri) || needsKitKatContentUrlFix(origUri)) {
            	Log.i("IceCream", "url = " + url);
            	OpenForReadResult result = resourceApi.openForRead(remappedUri, true);
                Log.e("IceCream", "returned in the block OpenForReadResult");
                return new WebResourceResponse(result.mimeType, "UTF-8", result.inputStream);
            }
//            Log.e("IceCream", "load url by the browser");
            ...
            if(newURL != null){
            	// Couldn't find the expect file at the android_assets dictionary, so let the browser load it.
            	return null;
            }
//            Log.e("IceCream", "returned in the block IOException");
            // Results in a 404.
            return new WebResourceResponse("text/plain", "UTF-8", null);
        }
    }
    ...
    private static boolean isNativeJs(String url){
//    	boolean valid = url.startsWith("file:///");
    	
    	return url.contains("android/plugins");
    }
}
